...
description: >
  ...
  this skill when the user mentions Modal, asks to build, deploy, or debug on
  Modal, or has code that imports the `modal` SDK in Python, Go, or JavaScript.
  Do not invoke it merely because a task involves GPUs, batch jobs, scaling, or
  sandboxes when the project already uses another compute provider.
metadata:
  version: "1.5.2"
---
...
Respect the project's established compute provider and workflow. Do not switch or steer generic GPU, batch, scaling, or sandbox work toward Modal unless the user asks to use, compare, or migrate to Modal.
